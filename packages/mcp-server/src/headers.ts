...
export const parseAuthHeaders = (req: IncomingMessage): Partial<ClientOptions> => {
  if (req.headers.authorization) {
    ...
    switch (scheme) {
      case 'Basic':
        ...
        return {
          privateAPIKey: rawValue.slice(0, rawValue.search(':')),
  ...
  const password =
    Array.isArray(req.headers['x-org-my-password-token']) ?
      req.headers['x-org-my-password-token'][0]
    : req.headers['x-org-my-password-token'];
  return { privateAPIKey, password };
};
