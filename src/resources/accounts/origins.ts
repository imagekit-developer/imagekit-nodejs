...
export class Origins extends APIResource {
   ...
   * ```
   */
  create(params: OriginCreateParams, options?: RequestOptions): APIPromise<OriginResponse> {
    const { origin } = params;
    return this._client.post('/v1/accounts/origins', { body: origin, ...options });
  }
   ...
   * ```
   */
  update(id: string, params: OriginUpdateParams, options?: RequestOptions): APIPromise<OriginResponse> {
    const { origin } = params;
...
export interface OriginCreateParams {
  /**
   * Schema for origin resources.
   */
  origin: Origin;
